...
def fix_broken_begin(text):
    ...
    text = re.sub(r'[\x00-\x1f\u200b\xa0]+imes', r'\\times', text)
...
def chinese_to_number(filename):
    """
    提取文件名中的分片序号，支持中文和阿拉伯数字
    """
    # 先找阿拉伯数字
    match = re.search(r'第(\d+)部分', filename)
    if match:
        return int(match.group(1))
    # 再找中文数字
    mapping = {"一": 1, "二": 2, "三": 3, "四": 4, "五": 5,
               "六": 6, "七": 7, "八": 8, "九": 9, "十": 10}
    match = re.search(r'第([一二三四五六七八九十]+)部分', filename)
    if match:
        ...
        if len(chinese_num) == 1:
            ...
            if chinese_num[0] == "十":
                ...
                return mapping.get(chinese_num[0], 0) * 10
            else:
                return mapping.get(chinese_num[0], 0) * 10 + mapping.get(chinese_num[1], 0)
    return 0